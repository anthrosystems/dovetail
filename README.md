...
result = d.task.run_blocking(fetch_data)

# Sync caller -> async function (blocks)
def get_remote():
    return d.task.run_blocking(fetch_data())

# Async caller -> sync function (await result)
result = await d.task.to_thread(fetch_data, "target.json")

# Async caller -> async function (await or fire-and-forget)
task = d.task.schedule(fetch_data())
```

### Quick API

- `d.task.to_thread(func, *args, **kwargs)` - run a blocking callable in the instance threadpool from async code.
- `d.task.schedule(coro_or_callable, *args, type=None, **kwargs)` - schedule a coroutine or execute a sync callable in the threadpool; returns an `asyncio.task`.
- `d.task.run_blocking(func_or_coro, *args, **kwargs)` - run sync functions or coroutines synchronously (uses `asyncio.run()`); raises if called inside a running event loop.

## Best practices

- Call `d.shutdown()` to cleanly close the threadpool when the instance is no longer needed.
